...
\begin{figure}[htbp]
    ...
    \caption{(Left) True signals: an edgy signal (top) and a smooth signal (bottom), and their corresponding blurred and noisy signals; (Middle) reduced functional $\varphi(\sigma)$ for selected values of $\lambda$; and (Right) convergence of $\sigma_k$ using the built-in MATLAB function \texttt{fminsearch}.}
...
Using the SVD of $\bA(\sigma)$ we can write
\begin{equation*}\bx(\sigma) = \bU (\bSigma^2 + \lambda^2 \bI)^{-1} \bSigma \bU^{\top} \bb.\end{equation*}
and use this expression of the solution $\bx(\sigma)$ in \eqref{eq: Tik-problem}. Then, defining $\widetilde{\bb} = \bU^\top \bb$, we have that
\begin{align*}
