...
\title[Hall $\pi$-subgroups and characters of $\pi'$-degree]{Hall $\pi$-subgroups and characters of $\pi'$-degree}
...
\thanks{Another thanks}

\begin{document}
\begin{abstract}
We study the relationship between the existence of Hall
$\pi$-subgroups and that of irreducible characters of $\pi'$-degree
...
Let $G$ be a finite group of order divisible by a prime $p$. Then
...
G$ is called a Hall $\pi$-subgroup of $G$ if $\pi(H)\subseteq \pi$
...
positive integer $k$, we let $\zeta_k:=e^{2\pi i/k}$ and we write $\QQ(\zeta_k)$ for the $k$th
...
\begin{lemma}\label{lem:noHall}
Let $n\in\mathbb{N}_{\geq 5}$ and let $\pi\subseteq [1,n]$ be a set
...
2$. In particular let $\pi=\{p_1,p_2,\ldots, p_t\}$ for some odd
...
that $H$ is also a non-solvable Hall $\pi$-subgroup of $\Sy_n$. This
...
|T^*| = (q-1)^{n-1},
\]
so $p$ divides $q-1$. In the unitary case,
\[
|T^*| =
\begin{cases}
...
\end{cases}
\]
and hence $p$ divides $q^2-1$ (see \cite[p.~74]{Carter85}). Since
...
of $\FF_{q}^\times$, and define the semisimple element
\[
\widetilde{s} := \diag\!\bigl( \delta^{(q-1)/p},
\delta^{(q-1)(p-1)/p}, 1^{\,n-2} \bigr) \in \widetilde{G}.
\]
Let $s$ denote the image $\pi(\widetilde{s})$ of $\widetilde{s}$
under $\pi:\widetilde{G}\to G^*$. Then
\[
\ord(s) = \ord(\widetilde{s}) = p.
\]
Taking conjugation if necessary, we may assume that $s\in T^*$.
