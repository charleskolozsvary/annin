...
from texpdfedits.extract import get_annots, get_edits, Annot, AnnotType
...
def main():
    ...
    if args.draw_annots:
        ...
        doc = pymupdf.open(filename)
        annots = get_annots(filename, adjust_annots=args.adjust_annots)
        drawRobustAnnots(filename, annots, bb_dir)

    # logging.info(f'Running get_edits({filename})...')
    edits = get_edits(filename, adjust_annots=args.adjust_annots)
    # logging.info("Done.")
