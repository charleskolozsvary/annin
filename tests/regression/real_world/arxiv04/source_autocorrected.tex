...
Many constructions in higher category theory allow one to build new $\infty$-categories out of existing ones: localization, stabilization, sheaf categories, the Lurie tensor product, etc. When the existing $\infty$-categories are presented by point-set models, it is natural to ask for a point-set model for the result of the construction. Here is an important example. Suppose that $(\mathbf C, \otimes)$ is a {monoidal} $\infty$-category. Let $\mathrm{Alg}(\mathbf C)$ denote the $\infty$-category of $\mathbb E_1$-algebras in $\mathbf C$. If $\mathbf C \simeq \mathsf C[\mathsf W^{-1}]$, can we write down a point-set model of $\mathrm{Alg}(\mathbf C)$ in terms of $\mathsf C$ and $\mathsf W$? Under certain hypotheses on $\mathsf C$ and $\mathsf W$, the answer is %%
%% Annotation 7, page 2 (AUTOCORRECTED) [ ]
%% Delete: "model categor<SEL>y,</SEL> which satisfies"
%% Comment: ""
%%
%% Annotation 8, page 2 (AUTOCORRECTED) [ ]
%% Delete: "satisfies either<SEL>:</SEL> (a) all"
%% Comment: ""
%%
%% Annotation 10, page 2 [ ]
%% Highlight: "satisfies either: <SEL>(a)</SEL> all objects"
%% Comment: "roman"
%%
%⭣ ⭣ ⭣ 
yes:

\begin{theorem}[Lurie] \label{lurie theorem} Suppose that $(\mathsf C,\mathsf W)$ underlies a combinatorial monoidal model categor which satisfies either (a) all objects %%
%⭡ ⭡ ⭡  END of annotations 7, 8, 10
%%
%% Annotation 6, page 2 (AUTOCORRECTED) [ ]
%% Replace: "are cofibrant, <SEL>(b)</SEL> it is"
%% Comment: "\textup{(b)}"
%%
%% Annotation 9, page 2 (AUTOCORRECTED) [ ]
%% Replace: "are cofibrant<SEL>,</SEL> (b) it"
%% Comment: " or"
%%
%⭣ ⭣ ⭣ 
are cofibrant or \textup{(b)} it is %%
%⭡ ⭡ ⭡  END of annotations 6, 9
left proper, symmetric monoidal, satisfies the monoid axiom, and is cofibrantly generated by cofibrations between cofibrant objects. %%
%% Annotation 5, page 2 (AUTOCORRECTED) [ ]
%% Delete: "≃Alg(C) [W−1<SEL>] ,</SEL>  where on"
%% Comment: ""
%%
%⭣ ⭣ ⭣ 
Then
	...
	where %%
%⭡ ⭡ ⭡  END of annotation 5
on the left we mean the $\infty$-category of $\mathbb E_1$-algebras %%
%% Annotation 4, page 2 (AUTOCORRECTED) [ ]
%% Delete: "in C[W−1<SEL>] ;</SEL> on the"
%% Comment: ""
...
%⭡ ⭡ ⭡  END of annotation 4
the right, we mean the 1-category of monoid objects in $\mathsf C$, localized at the class of arrows whose underlying morphism is in the class $\mathsf W$. 	
\end{theorem}
