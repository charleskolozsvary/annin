...
\title[Multiple orthogonality on the unit circle]{Szeg\H{o} mapping and Hermite--Pad\'{e} polynomials for multiple orthogonality on the unit circle}

\author{Rostyslav Kozhan}
\email{kozhan@math.uu.se}

\author{Marcus Vaktn\"as}
...
The Laurent polynomials described in   (i), (ii),  (iii), and (iv) will be called type II, type II$^*$, I, and I$^*$, respectively. For the case of indices of the form $(\bm{n};-\bm{n})$ (for any $\bm{n}\in\Z^r$), which are all normal, it is natural to take $\Phi_{\bm{n};-\bm{n}}(z) = \Phi^*_{\bm{n};-\bm{n}}(z) = 1$, $\bm{\Xi}_{\bm{n};-\bm{n}}(z) = \bm{\Xi}^*_{\bm{n};-\bm{n}}(z) = \bm{0}$. 
